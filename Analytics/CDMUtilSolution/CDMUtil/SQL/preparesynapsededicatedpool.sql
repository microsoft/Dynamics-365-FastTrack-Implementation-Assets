...
IF NOT  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DataLakeToSynapse_ControlTableForCopy]') AND type in (N'U'))
...
END
GO
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.DataLakeToSynapse_CopyIntoSingleTable') AND type in (N'P'))
drop PROC dbo.DataLakeToSynapse_CopyIntoSingleTable
GO
Create  PROC [dbo].[DataLakeToSynapse_CopyIntoSingleTable] @TableName [varchar](255), @Schema varchar(50) AS
BEGIN
declare @TableToCopy varchar(255);
declare @FilePath varchar(500);
declare @FileFormat varchar(100);
SELECT top 1 @TableToCopy = TableName, @FilePath= DataLocation, @FileFormat  = FileFormat  
FROM [dbo].[DataLakeToSynapse_ControlTableForCopy] 
WHERE TableName =  @Schema + '.' + @TableName;
select @TableToCopy, @FilePath, @FileFormat
Exec dbo.DataLakeToSynapse_CopyIntoTable @TableToCopy, @FilePath, @FileFormat;
END
