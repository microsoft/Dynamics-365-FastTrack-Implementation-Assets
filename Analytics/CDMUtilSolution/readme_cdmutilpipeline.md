...
Note: You do not have to create Dedicated SQL pool to deploy the pipeline if you do not plan to use dedicated SQL pool. Just select the default Synapse workspace linked service that you will find with a name like **{WorkspaceName}-WorkspaceDefaultSqlServer**
...
Note: CDMUtil pipeline only creates metadata (Tables) and populated metadata information in the control table. To copy data in Synapse tables, ADF or Synapse pipelines can be used.   
...
3. Import Synapse pipeline template ![Import Synapse pipeline template](importsynapsepipelinetemplate.png)
4. Provide parameters and execute DataLakeToSQLCopy pipeline to copy data to SQL tables.

DataLakeToSQLCopy reads the control table to collect metadata and then uses DataFlow to copy full and incremental data to SQL server.  
