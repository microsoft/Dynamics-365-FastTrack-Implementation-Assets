select avg(nbqueries) nbqueriesperhour
from
(
	select 
		sum(count_executions) nbqueries, CAST(FORMAT(rsi.start_time,'yyyy-MM-dd HH:0') AS datetime) hourofDay  
	FROM 
	...
	group by 
		CAST(FORMAT(rsi.start_time,'yyyy-MM-dd HH:0') AS datetime)
) nbqueriesperhour