...
param(
    ...
    [string]$AlertRuleName = 'store-monitoring-database-size',

    [int]$DatabaseSizeThresholdMB = 8192,

    [string]$QueryLookback = '24h',

    [int]$Severity = 2,

    [string]$EvaluationFrequency = 'PT24H',

    [string]$WindowSize = 'PT24H',
...
$templateFile = Join-Path $deployRoot 'database-size-alert.bicep'
...
$deploymentName = 'store-monitoring-database-size-alert-{0}' -f (Get-Date -Format 'yyyyMMddHHmmss')
...
$parameters = @{
    ...
    parameters     = @{
        ...
        databaseSizeThresholdMB = @{ value = $DatabaseSizeThresholdMB }
        queryLookback           = @{ value = $QueryLookback }
