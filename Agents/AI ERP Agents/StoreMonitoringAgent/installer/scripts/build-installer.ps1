...
# This script builds the Database Metrics Service MSI installer using WiX v4 SDK
...
$ServiceDir = Join-Path $RepoRoot "services\DatabaseMetricsService"
...
Write-Host "  Building Database Metrics Service Installer" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "[2/4] Building DatabaseMetricsService..." -ForegroundColor Yellow
Push-Location $ServiceDir
try {
    dotnet publish -c $Configuration -r win-x64 --self-contained false
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to build DatabaseMetricsService"
    }
    Write-Host "  DatabaseMetricsService built successfully" -ForegroundColor Green
}
finally {
    Pop-Location
}
...
\f0\fs24 Database Metrics Service License Agreement\par
\par
...
if ($MsiPath) {
    ...
    Write-Host "  $WixProjectDir\bin\$Configuration\x64\DatabaseMetricsService.Installer.msi" -ForegroundColor White
}
Write-Host ""
