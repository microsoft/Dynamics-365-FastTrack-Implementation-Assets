-- =============================================================
-- CLEAN-INSTALL GUARD
-- This is a from-scratch install: existing objects are dropped so
-- the latest schema is deployed every time the script runs.
-- WARNING: re-running this script WIPES all data in these tables.
-- Views are dropped first, then tables (no FK dependencies exist).
-- =============================================================
DROP VIEW IF EXISTS [dbo].[PayRecon_PolicyDrivenRules]
GO
DROP VIEW IF EXISTS [dbo].[ReconciliationPolicy]
GO
DROP VIEW IF EXISTS [dbo].[PayRecon_UnreconciledCommerceTransactions]
GO
DROP VIEW IF EXISTS [dbo].[PayRecon_UnreconciledPaymentProcessorTransactions]
GO
DROP TABLE IF EXISTS [dbo].[CommercePaymentTransactionsStaging]
GO
DROP TABLE IF EXISTS [dbo].[CommercePaymentTransactions]
GO
DROP TABLE IF EXISTS [dbo].[CommercePaymentTransactionsHistory]
GO
DROP TABLE IF EXISTS [dbo].[PaymentProcessorTransactionsStaging]
GO
DROP TABLE IF EXISTS [dbo].[PaymentProcessorTransactions]
GO
DROP TABLE IF EXISTS [dbo].[PaymentProcessorTransactionsHistory]
GO
DROP TABLE IF EXISTS [dbo].[ReconciliationExecution]
GO
DROP TABLE IF EXISTS [dbo].[ReconciliationRules]
GO
...
/****** Object:  View [dbo].[ReconciliationPolicy] ******/
-- Compatibility alias: the Power App and agents reference [dbo].[ReconciliationPolicy]
-- (the original name of the rules table, since renamed to ReconciliationRules).
-- This view keeps those bindings working without editing the app.
CREATE VIEW [dbo].[ReconciliationPolicy] AS
SELECT * FROM [dbo].[ReconciliationRules];
GO