...
-- Clean-install guard: clear existing rules so the latest rule
-- set is seeded every time without duplicating rows on re-run.
DELETE FROM [dbo].[ReconciliationRules];
GO
